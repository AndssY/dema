...
class S6(nn.Module):
    def __init__(
        ...
        self.d_conv = d_conv
        self.expand = 2
        self.d_inner = hidden_size
        self.d_model = int(self.d_inner/self.expand)
        ...
        self.act = nn.SiLU()
        # self.in_proj = nn.Identity(batched=True)
        ...
        # self.out_proj = nn.Identity(batched=True)
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        if config.attn_type == 'attn':
            self.attn = Attention(hidden_size, n_ctx, config, scale)
        elif config.attn_type == 's6':
            self.attn = S6(hidden_size)
